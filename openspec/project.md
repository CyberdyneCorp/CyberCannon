...
The spec deltas hold **666 GIVEN/WHEN/THEN scenarios**. They are not documentation:
...
| File | What it is |
|---|---|
| `design/cybercanon-neo-brutal.html` | The design source. Its inline `:root` is the token values; the component classes it rides on are `design/_ds/broadsheet-*/styles.css`. |
| `apps/cybercanon/web/src/lib/styles/tokens.css` | The one token file. Every custom property the design declares, transcribed; plus the values the design writes literally in its component layer (border weights, the spot fills, the intermediate shadows), named so a component never has to. |
| `apps/cybercanon/web/src/lib/styles/base.css` | The base layer — ground, type, links, controls, `:focus-visible`, `::selection`, disabled. Themed once, per the system's readme, never restyled per page. |
| `apps/cybercanon/web/src/lib/styles/fonts.css` + `static/fonts/` | Archivo, Space Grotesk and Space Mono, **self-hosted**. Generated by `scripts/vendor_fonts.py`. |
| `apps/cybercanon/web/src/routes/+layout.svelte` | The only place those three are imported. |
| `apps/cybercanon/web/scripts/adherence.mjs` | The gate. `just adherence`, and `just check` runs it. |
| `apps/cybercanon/web/adherence.json` | The components still awaiting the restyle. The list is only allowed to shrink. |

**The fonts are self-hosted, and that is a decision rather than a default.** The
design's own page links Google Fonts. This interface is read on studio machines
behind networks that block third-party CDNs, and a stylesheet request that hangs
does not degrade the page — it blocks first paint for as long as the browser is
willing to wait. Vendored, the worst case is a font that is simply there. Each
family still carries a real fallback stack behind it, so a face that fails to
load costs the page its voice and never its layout.

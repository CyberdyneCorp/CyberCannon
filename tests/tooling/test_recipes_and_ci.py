...
STATED_COUNTS: tuple[tuple[Path, re.Pattern[str]], ...] = (
    (Path("openspec") / "project.md", SPEC_COUNT),
    (Path("ROADMAP.md"), re.compile(r"(?P<count>\d+) scenarios \(\d+ executing")),
    (Path("README.md"), re.compile(r"Spec deltas - (?P<count>\d+) scenarios")),
)
"""Every file that states the size of the corpus in prose.

Three of them, because a number a person maintains by hand drifts silently and
each of these had: `project.md` said 670 against a corpus of 672, and the other
two still said 657. A reader has no way to tell a stale count from a current
one, and the roadmap's in particular read as 78% of the work done when it was
98%. Pinning them all to the generated report is what stops the next edit to a
spec delta quietly making three documents wrong.
"""
...
@pytest.mark.parametrize(("document", "pattern"), STATED_COUNTS, ids=lambda value: str(value)[:24])
def test_every_document_that_states_the_corpus_size_states_the_current_one(
    repo_root: Path, document: Path, pattern: re.Pattern[str]
) -> None:
    """A count kept by hand against a corpus generated by a script.

    The corpus is the fact and the sentence follows it, so this compares each
    document against the report `just check` regenerates rather than against
    the other documents — which would let all three be wrong together.
    """
    ...
    stated = pattern.search((repo_root / document).read_text(encoding="utf-8"))
    assert stated is not None, f"{document} no longer states the scenario count"

    assert int(stated.group("count")) == int(summary.group("total")), (
        f"{document} says {stated.group('count')} scenarios; the corpus has "
        f"{summary.group('total')}. The corpus is the fact."
    )
